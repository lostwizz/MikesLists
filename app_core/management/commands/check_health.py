...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # logger.traceb(f"{request=}")
        # Simulate auth
        user = User.objects.filter(is_superuser=True).first()
        if not user:
            self.stdout.write(
                self.style.ERROR(
                    "Critical Failure: No superuser found to authenticate request."
                )
            )
            return

        request.user = user
        # logger.tracee(f"{request.user=}")
        try:
            response = health(request)
            data = json.loads(response.content)

            # logger.tracek(f"{data=}")
            ...
            for c in checks.values():
                ...
                # logger.traceu("gone insane")
                self.stdout.write(color(f"[{status:<7}] "), ending="")
                self.stdout.write(f"{msg[:25]:<25}", ending="")
                self.stdout.write(f"{raw[:70]:<70}")

            self.stdout.write("-" * 45)
            final_color = (
                self.style.SUCCESS if overall == "HEALTHY" else self.style.ERROR
            )
